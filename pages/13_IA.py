...
with col_help:
    ...
    <div class="example-box">
        <strong>🔍 Ejemplos que puedes probar:</strong>
        <ul>
            <li>Dame casos de <b>diarrea</b> en <b>Jujuy</b> entre <b>2020</b> y <b>2022</b></li>
            <li>Muestra <b>bronquiolitis</b> en <b>Salta</b> del <b>2023</b></li>
            <li><b>Influenza</b> en <b>Buenos Aires</b> en <b>2021</b></li>
            <li>Casos de <b>IRAG</b> en el <b>norte</b> argentino en <b>2022</b></li>
            <li>Total de <b>neumonía</b> en <b>Córdoba</b> <b>2019</b></li>
            <li><b>ETI</b> en la <b>patagonia</b> desde <b>2018</b> hasta <b>2020</b></li>
        </ul>
...
with col_input:
    ...
    query_text = st.text_area(
        ...
        height=120,
        placeholder="Ejemplo: dame casos de dengue en Salta del 2023",
        help="Escribe tu consulta en lenguaje natural. El sistema detectará automáticamente provincias, eventos y años."
    )
...
def get_query_builder():
    return NaturalLanguageQueryBuilder()
        ...
        if params['success']:
            ...
            with col1:
                if params.get('provincias'):
                    st.markdown(f"**📍 Provincias:** {', '.join(params['provincias'])}")
                else:
                    st.markdown("**📍 Provincias:** Todas")
            ...
            if not df_result.empty:
                ...
                st.dataframe(
                    df_result.head(10).style.background_gradient(cmap="Blues", subset=['CANTIDAD']),
                ...
                if viz_type == "Barras por Provincia":
                    df_prov = df_result.groupby('PROVINCIA', as_index=False)['CANTIDAD'].sum()
                    fig = px.bar(
                        df_prov.sort_values('CANTIDAD', ascending=False),
                        x='PROVINCIA',
                        y='CANTIDAD',
                    ...
                    df_tiempo = df_result.groupby('ANIO', as_index=False)['CANTIDAD'].sum()
                    fig = px.line(
                        df_tiempo.sort_values('ANIO'),
                        x='ANIO',
                        y='CANTIDAD',
                ...
                # Opcionalmente mostrar la query SQL generada
                with st.expander("🔧 Ver consulta SQL generada"):
                    st.code(params.get('sql_query', ''), language='sql')
...
elif consultar_btn and not query_text.strip():
...
with st.expander("Ver detalles técnicos"):
    ...
    **Sistema de Consultas Basado en Reglas**
    ...
    1. **Provincias:** Detecta nombres de provincias argentinas y sus variaciones (ej: Tucumán/Tucuman)
    2. **Regiones:** Reconoce regiones como "norte", "sur", "patagonia", "cuyo", etc.
